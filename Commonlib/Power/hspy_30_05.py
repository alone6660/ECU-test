...
class HSPY3603PowerController:
    ...
    def set_voltage(self, voltage):
        """设置输出电压"""
        voltage_int = int(voltage * 100)  # 转换为整数，保留2位小数
    ...
    def set_current(self, current):
        """设置输出电流"""
        current_int = int(current * 1000)  # 转换为整数，保留3位小数
    ...
    def get_voltage_display(self):
        ...
        if response and len(response) >= 7:
            # 修复：正确解析bytes类型的响应数据
            # 响应格式：地址(1) + 功能码(1) + 字节数(1) + 数据(2) + CRC(2)
            voltage_raw = (response[3] << 8) | response[4]
            return voltage_raw * 0.01  # 转换为实际电压值
        return None
    ...
    def get_current_display(self):
        ...
        if response and len(response) >= 7:
            ...
            return current_raw * 0.001  # 转换为实际电流值
        return None
    ...
    def get_power_info(self):
        """获取完整的电源信息"""
        info = {}
        info['voltage_display'] = self.read_voltage_display()
        info['current_display'] = self.read_current_display()
        info['current_setting'] = self.read_current_setting()
        info['power_status'] = self.read_power_status()
        return info
...
if __name__ == "__main__":
    ...
    #             print(f"输出电压: {power_info['voltage_display']:.2f}V")
    #         else:
    #             print("输出电压: 读取失败")
    #
    #         if power_info['current_display'] is not None:
    #             print(f"输出电流: {power_info['current_display']:.3f}A")
    #         else:
    #             print("输出电流: 读取失败")
    #
    #         if power_info['current_setting'] is not None:
    #             print(f"电流设定: {power_info['current_setting']:.3f}A")
    #         else:
    #             print("电流设定: 读取失败")
    #
